...
export const postBancal = async (req, res) => {
    try {
        ...
        const sql = `
            INSERT INTO bancal (nombre, TamX, TamY, posX, posY, fk_lote) 
            VALUES ($1, $2, $3, $4, $5, $6) 
            RETURNING id
        `;
...
export const getBancal = async (req, res) => {
    try {
        const sql = `
            SELECT id, nombre, TamX, TamY, posX, posY, fk_lote 
            FROM bancal 
            ORDER BY nombre
        `;
...
export const updateBancal = async (req, res) => {
    try {
        ...
        const sql = `
            UPDATE bancal 
            SET nombre = $1, TamX = $2, TamY = $3, posX = $4, posY = $5, fk_lote = $6 
            WHERE id = $7
        `;
...
export const deleteBancal = async (req, res) => {
    try {
        const { id } = req.params;
        const sql = "DELETE FROM bancal WHERE id = $1";
        const result = await pool.query(sql, [id]);
        if (result.rowCount > 0) {
