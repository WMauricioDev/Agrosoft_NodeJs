...
const PrecioProductoPage: React.FC = () => {
  ...
  const { data: cosechas, isLoading: isLoadingCosechas } = useCosechas();
  ...
  const handleSubmitUnidadMedida = () => {
    if (!nuevaUnidad.nombre) {
      addToast({
        title: "Error",
        description: "El nombre de la unidad de medida es requerido",
        timeout: 3000,
      });
      return;
    }
  ...
  // Log para depurar los datos de cosechas
  React.useEffect(() => {
    console.log("Datos de cosechas:", cosechas);
  }, [cosechas]);

  if (isLoadingCosechas || isLoadingUnidades) {
      ...
      >
        <div>
          ...
          <select
            ...
            onChange={(e) => {
              const cosechaId = e.target.value ? Number(e.target.value) : null;
              const selectedCosecha = cosechas?.find(c => c.id === cosechaId);
              console.log("Cosecha seleccionada:", selectedCosecha);
              setPrecioProducto({
                ...precioProducto,
                cosecha: cosechaId,
                nombre_cultivo: selectedCosecha?.cultivo_nombre || "Cultivo Desconocido",
              });
            }}
          ...
          >
            ...
            {cosechas?.map((cosecha) => (
              <option key={cosecha.id} value={cosecha.id}>
                {`Cosecha ${cosecha.id_cultivo_id} ${cosecha.cultivo_nombre || 'Cultivo Desconocido'} - ${cosecha.fecha || 'Sin fecha'}`}
              </option>
            ))}
          </select>
        </div>
